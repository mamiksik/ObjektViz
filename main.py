...
def generate_token_animation_segments(data: list[dict], start_date, end_date, animation_preferences: TokenReplayPreferences):
    ...
    for trace in data:
        ...
        if animation_preferences.fixed_animation_duration:
            segments = [ReplaySegment(
                ...
                start_offset_sec=i,
                duration_sec=1,
                activity_duration_sec=x.get("DurationSec") * 0, # TODO: activity_animation
                color="#3e9b0a ",
        ...
        replay_duration = segments[-1].start_offset_sec + segments[-1].duration_sec + segments[-1].activity_duration_sec
...
@st.cache_data
def trace_variants(class_type: str, variant: Literal['DoA', 'PreventedDoA', 'DoAMaterialType'], limit: int, part_family_name: str|None):
    return []
...
graph_tab, trace_variants_tab, debug_tab = st.tabs(["Graph", 'Trace Variants', "Debug tab"])
...
with st.sidebar:
    with st.expander("Layout preferences", expanded=False):
        layout_preferences = ov_components.layout_preferences_input(CLUSTERING_ATTRS, CLUSTERING_ATTRS_DEFAULT)

    with st.expander("Connection preferences", expanded=False):
        edge_vis_preferences = ov_components.edge_render_preference_input(queries.dfc_attributes(class_type), default='count')
    ...
    with st.expander("Entities to animate", expanded=True):
        element_id_filter, active_element_ids, token_animation_segments = ov_filters.DummyFilter.new(is_passing=False), None, None
        selected_entity_ids = ov_components.token_replay_input(class_type, ENTITY_ID_SAMPLER)

        replay_metadata = None
        if len(selected_entity_ids) != 0:
            result = queries.get_process_executions(class_type, selected_entity_ids)
            active_element_ids, token_animation_segments, replay_metadata = generate_token_animation_segments(result[0], result[1], result[2], animation_preferences)
            element_id_filter = ov_filters.MatchFilter.new(attribute='element_id', is_enabled=True, skip_on_empty=True,
    ...
    with st.expander("Activity filters", expanded=False):
        root_node_filter = ov_filters.MatchFilter.new(
            ...
            is_enabled=True,
            skip_on_empty=True,
            values=st.multiselect("Entity types to show", options=ENTITY_TYPES, default=ENTITY_TYPES)
        )
...
wrapped_values = ov_neo4j.from_neo4j_to_dot_elements(nodes, edges, dot_visualizer_config)
