...
class OcelImportQueryLibrary:
    ...
    def q_define_table(node_label, csv_header):
        ...
        event_ddl = ', '.join(
            f"{col} {col_types.get(col, 'STRING')}" for col in csv_header
        ) + ", PRIMARY KEY(id)"
    ...
    def q_define_rel(rel_label, from_node, to_node, attributes):
        ...
        query_str = f"CREATE REL TABLE {rel_label} (FROM {from_node} TO {to_node} {attrs})"
        return query_str

    @staticmethod
    def q_load_csv_as_relation(fileName, csvFrom, sourceNode, sourceAttr, csvType, relationship, csvTo, targetNode, targetAttribute) -> str:
        query = f'''
            LOAD FROM $dataframe
