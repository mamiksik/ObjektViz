...
from objektviz.backend.dot_elements import AbstractDotElement, DotEdge, DotNode, CROSS_CLUSTER_SENTINEL
...
def generate_dot_source(
    ...
    for edge in edges:
        ...
        if (edge.start_element_id not in visible_node_ids) or (edge.end_element_id not in visible_node_ids):
            continue

        if (
            (edge.start_element_id, edge.end_element_id) in sync_edges_ids
            or (edge.end_element_id, edge.start_element_id) in sync_edges_ids
        ):
            continue
    ...
    for node in nodes:
        if node.element_id not in connected_nodes_ids or node.element_id not in visible_node_ids:
            continue
    ...
    for edge in edges:
        ...
        if (
            edge.is_sync_edge
            and not (edge.start_element_id in connected_nodes_ids and edge.end_element_id in connected_nodes_ids)
        ):
            continue
...
class DotGraphDescriptorBuilder:
    def __init__(self, *, config: BackendConfig, edge_shaders: dict[str, AbstractShader],
                 visible_node_ids: set[str], edge2node: dict[str, list[str]], node2edge: dict[str, list[str]],
                 node2node: dict[str, list[str]], node_rank: dict[str, list[str]]
                 ):
        self.config = config
    ...
    def build_subgraph(
        self, graph: graphviz.Digraph, items: list[AbstractDotElement], level,
    ):
        """
        ...
        if level >= len(self.config.layout_preferences.clustering_keys):
            if self.config.start_end_nodes_per_cluster and self.config.visualize_start_end_flag:
                self.inject_local_start_end_nodes(graph, items)
    ...
    def inject_local_start_end_nodes(self, graph: graphviz.Digraph, items: list[AbstractDotElement]):
        """
        ...
        if local_start_nodes:
            graph.node(id=local_start_id, name=local_start_id, label="▼", shape="circle", style='filled', color='#00800070', fontcolor='#008000', fillcolor='#90c08c')
            self.start_nodes_ids.append(local_start_id)

            for node in local_start_nodes:
                edge_id = f"{local_start_id}-{to_lbl(node.element_id)}"

                self.edge2node.setdefault(edge_id, []).extend([local_start_id, node.element_id])
                ...
                pen_width = self.edge_shaders[node.shader_key].pen_width({self.config.event_class_preferences.shading_attr: node.process_start_count})
                graph.edge(local_start_id, to_lbl(node.element_id), id=edge_id, label=f"{node.process_start_count:,}", penwidth=str(pen_width), style='dashed', color='#00800050', arrowsize='2')

        if local_end_nodes:
            graph.node(id=local_end_id, name=local_end_id, label="✖", shape="circle", color='#B31B1B70', style='filled', fontcolor='#B31B1B', fillcolor='#e2968d')
            self.end_nodes_ids.append(local_end_id)

            for node in local_end_nodes:
                edge_id = f"{to_lbl(node.element_id)}-{local_end_id}"

                self.edge2node.setdefault(edge_id, []).extend([edge_id, node.element_id])
                ...
                pen_width = self.edge_shaders[node.shader_key].pen_width({self.config.event_class_preferences.shading_attr: node.process_end_count})
                graph.edge(to_lbl(node.element_id), local_end_id, id=edge_id, label=f"{node.process_end_count:,}", penwidth=str(pen_width), style='dashed', color='#B31B1B50', arrowsize='2')
    ...
    def enforce_same_rank(self, graph):
        if self.config.visualize_start_end_flag and self.config.layout_preferences.force_process_start_end_same_rank:
            elements = ';'.join([f'"{e}"' for e in self.start_nodes_ids])
            graph.body.append(
                f'{{rank=same; {elements};}};'
            )
