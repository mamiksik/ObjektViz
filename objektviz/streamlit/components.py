...
from objektviz.streamlit.utils import DefaultConnectionPreferences, DefaultEventClassPreferences, \
    DefaultLayoutPreferences, TokenReplayManager
from objektviz.backend.dot_elements import DotNode, DotEdge
from objektviz.frontend import TokenReplayPreferences, Token, interactive_proclet_graph, wire_graph_event, ReplayMetadata
...
def general_preferences(proclet_types: list[str]) -> tuple[str, bool, bool, bool, Callable[['BackendConfig', str, str], ov_shaders.AbstractShader]]:
    with st.expander("General preferences"):
        class_type = st.selectbox("Proclet class type", options=proclet_types)

        is_process_start_end_visualized = st.toggle("Show process start/end nodes", value=True)
        start_end_nodes_per_cluster = st.toggle("Start and end nodes per cluster", value=True, disabled=not is_process_start_end_visualized)

        enable_path_effects_on_hover = st.toggle("Enable path effects on hover", value=True)
        shader_factory = builtin_shader_selector()

        return class_type, is_process_start_end_visualized, start_end_nodes_per_cluster, enable_path_effects_on_hover, shader_factory
...
def edge_render_preference_input(edge_attributes: list[str], defaults: DefaultConnectionPreferences) -> ConnectionPreferences:
    return ConnectionPreferences(
        pen_width_range=st.slider("Pen with range", min_value=1, max_value=30, value=defaults.pen_range),
        caption=st.selectbox("Edge attr", options=edge_attributes, index=edge_attributes.index(defaults.title)),
        shading_attr=st.selectbox("Edge shading", options=edge_attributes, index=edge_attributes.index(defaults.shading)),
        use_x_labels=st.toggle("Use xlabels [labels are ignored while routing]")
    )


def node_render_preference_input(node_attributes: list[str], defaults: DefaultEventClassPreferences) -> EventClassPreferences:
    return EventClassPreferences(
        shading_attr=st.selectbox("Node shading", options=node_attributes, index=node_attributes.index(defaults.shading_attr)),
        title=st.selectbox("Node tile", options=node_attributes, index=node_attributes.index(defaults.title)),
        caption_left=st.selectbox("Node caption left", options=node_attributes, index=node_attributes.index(defaults.small_caption_left)),
        caption_right=st.selectbox("Node caption right", options=node_attributes, index=node_attributes.index(defaults.small_caption_right)),
...
def layout_preferences_input(
    ...
    return LayoutPreferences(
        force_same_rank_for_event_class=cont.checkbox("Force same rank per activity", value=False),
        force_process_start_end_same_rank=cont.checkbox("Force same rank for process estart/end", value=False),
        sort_event_classes_by_frequency=cont.checkbox('Sort nodes by frequency', value=True),
        sort_connections_by_frequency=cont.checkbox('Sort edges by frequency', value=True),
        node_separation=col1.number_input("Node separation", min_value=0.1, max_value=5.0, step=0.1, value=0.5),
        rank_separation=col2.number_input("Rank separation", min_value=0.1, max_value=5.0, step=0.1, value=0.5),
        clustering_keys=(
            st.multiselect("Clustering attrs [ordered]", options=default_layout_preferences_input.allowed_clustering_attributes, default=default_layout_preferences_input.default_clustering_attribute)
            if st.toggle("Enable clustering", value=True) else []
        ),
    )
    ...
    *,
...
) -> tuple[LayoutPreferences, ConnectionPreferences, EventClassPreferences, ov_filters.AbstractFilter, TokenReplayPreferences]:
    ...
    with st.expander("DFC Appearance", expanded=False):
        edge_vis_preferences = edge_render_preference_input(
            ...
            defaults=default_connection_visuals
        )
    ...
    with st.expander("Event Class Appearance", expanded=False):
        node_vis_preferences = node_render_preference_input(
            ...
            defaults=default_event_class_visuals
        )

    with st.expander("Animation preferences", expanded=False):
        show_only_sampled_elements = ov_filters.DummyFilter.new(
            is_passing=not st.toggle("Hide connections and classes not contained in the sample")
        )
        animation_preferences = animation_preferences_input()

    return layout_preferences, edge_vis_preferences, node_vis_preferences, show_only_sampled_elements, animation_preferences
...
def animation_preferences_input() -> TokenReplayPreferences:
    result =  TokenReplayPreferences(
        ...
        token_animation_speed=25.5 - st.slider("Animation speed", value=5.0, min_value=0.1, max_value=25.0, step=0.1),
        token_animation_alignment=(alignment := st.segmented_control(
            "Animation time alignment", options=["At-once", "Real-time"], default="Real-time"
        )),
        fixed_animation_duration=st.toggle("Fixed transition duration", value=False, disabled=(alignment != "At-once")),
    )
...
def token_replay_input(
...
) -> tuple[ov_filters.AbstractFilter, list[Token] | None, ReplayMetadata | None, list[str] | None]:
    with st.expander("Entities to animate", expanded=True):
        ...
        for name, callback in ui_preferences.samplers.items():
            ...
            sample_size = col2.number_input("No. Samples", value=10, min_value=1, max_value=2500, key=f'{name}_sample',
        ...
        active_element_ids, token_animation_segments, replay_metadata = ui_preferences.token_animation_generator(sampled_traces[0], sampled_traces[1], sampled_traces[2], token_replay_preferences)
        element_filter = ov_filters.MatchFilter.new(attribute='element_id', is_enabled=True, skip_on_empty=True,
                                                       values=active_element_ids)

        return element_filter, token_animation_segments, replay_metadata, active_element_ids


def attribute_range_filter_input(label: str, entity_type: str, attr: str, rng: tuple[int, int], default_is_enabled=False):
    ...
    return ov_filters.OrFilter.new([
        ov_filters.NotFilter.new(ov_filters.MatchFilter.new(attribute='EntityType', values=[entity_type])),
        range_filter
    ])


def animation_segments(token_animation_segments: list[Token]):
    entity_selector = st.selectbox("Select animation entity", options=map(lambda value: value.entity_id, token_animation_segments), index=0)
...
def event_class_detail(
    ...
    node = next((node for node in nodes if node.element_id == selected_element_id), None)
    if node is None:
        return
...
def event_class_related_entities(
    ...
    limit = col2.number_input("Entities per page", min_value=1, max_value=100, value=100, step=1, key="entities_per_page_ec")
    options = [f"{limit * (x - 1)}-{x * limit + 1}" for x in range(1, entity_count // limit + 2)]
    selection = col1.segmented_control("Select page", options=options, default=options[0])
...
def dfc_detail(
    ...
    edge = next((edge for edge in edges if edge.element_id == selected_element_id), None)
    if edge is None:
        return
...
def dfc_related_entities(
    ...
    # Pagination inputs
    col1, col2 = st.columns([0.8, 0.2])
    limit = col2.number_input("Entities per page", min_value=1, max_value=100, value=100, step=1, key="entities_per_page_dfc")
    options = [f"{limit * (x - 1)}-{x * limit + 1}" for x in range(1, entity_count // limit + 2)]
    selection = col1.segmented_control("Select page", options=options, default=options[0])
    offset = int(selection.split("-")[0])
...
def full_proclet_view(*, graph_payload, nodes, edges, queries, class_type, token_animation_segments):
    ...
    with tab1:
        ...
        if st.session_state.selected_node:
            event_class_detail(nodes, queries.class_attributes(class_type), st.session_state.selected_node)
            event_class_related_entities(queries, st.session_state.selected_node)

        if st.session_state.selected_edge:
            dfc_detail(edges, queries.dfc_attributes(class_type), st.session_state.selected_edge)
...
def ekg_stats(queries: AbstractEKGRepository, class_type: str):
    ...
    col1.metric('Activity Classes Present', queries.count_classes(class_type), help="Number of distinct Activity classes in the selected proclet")
    col2.metric('Activity Connections Present', queries.count_dfc(class_type), help="Number of distinct Activity connections in the selected proclet")
    col3.metric("SYNC Edges Present", queries.count_sync(class_type), help="Number of SYNC edges in the selected proclet")

    col1, col2, col3 = st.columns(3)
    col1.metric('Process Start/End present', "StartCount" in class_attrs and "EndCount" in class_attrs, help="Whether at least some Activity nodes have **StartCount** and **EndCount** set")
    col2.metric('Start Activities', queries.count_start_activities(class_type), help="Number of distinct start Activity classes in the selected proclet")
    col3.metric('End Activities', queries.count_end_activities(class_type), help="Number of distinct end Activity classes in the selected proclet")
